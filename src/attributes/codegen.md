...
在此列表中标记为"(仅限 cfg)"的特性名称只能与 [`target_feature`][cfg.target_feature] 条件编译选项一起使用，不能与 `target_feature` 属性一起使用。
...
有关这些特性的更多文档可以在各自的规范中找到。许多规范在 [RISC-V ISA Manual][version 20250508] 或 [RISC-V GitHub Account] 上托管的其他手册中进行了描述。
