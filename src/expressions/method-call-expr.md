...
方法调用被解析为特定 trait 上的关联[方法][methods]，如果左侧的精确 `self`-类型已知，则静态分派到方法；如果左侧表达式是间接的 [trait 对象](../types/trait-object.md)，则动态分派。
...
第一步是构建一个候选接收者类型列表。通过重复[解引用][dereference]接收者表达式的类型来获取这些类型，将遇到的每个类型添加到列表中，然后最后尝试一次数组[非固定大小强制转换][unsized coercion]，如果成功则添加结果类型。
...
如果到达某一步时存在多个可能的方法，例如泛型方法或 trait 被视为相同，则这是编译错误。这些情况需要[消歧函数调用语法][disambiguating function call syntax]来进行方法和函数调用。

r[expr.method.edition2021]
> [!EDITION-2021]
> 在 2021 版本之前，在搜索可见方法的过程中，如果候选接收者类型是[数组类型][array type]，则标准库 [`IntoIterator`] trait 提供的方法会被忽略。
>
...
> [!WARNING]
> 对于 [trait 对象][trait objects]，如果存在与 trait 方法同名的固有方法，则在方法调用表达式中尝试调用该方法时将给出编译错误。相反，你可以使用[消歧函数调用语法][disambiguating function call syntax]调用该方法，在这种情况下，它调用的是 trait 方法，而不是固有方法。无法调用固有方法。只要不在 trait 对象上定义与 trait 方法同名的固有方法，就不会有问题。
